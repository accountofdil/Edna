...
@st.cache_resource
def load_artifacts():
    with open('Demand Forecasting Model - XGBoost Model.pkl', 'rb') as f:
        model = pickle.load(f)
    with open('Demand Forecasting Model - Label Encoders.pkl', 'rb') as f:
...
input_data = pd.DataFrame({'Price': [price],
                           'Discount': [discount],
